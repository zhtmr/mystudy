...
public class Controller03_2 {
  ...
  @ResponseBody
  public String handler1(Model model, HttpSession session /* getSession() 호출된 상태로 핸들러가 실행된다. */) {
  ...
  public String handler2(HttpSession session) {
    ...
    /* /h3 -> /h1 하면 세션에 보관이 안되는 이유 ! */
    // getSession() 호출 할때 세션이 만들어진다! 세션 무효화(/h3)을 하게되면 세션이 없어진다.
    // 이 상태에서 세션에 값을 넣으려고 해봤자(/h1) 세션이 만들어지지 않아서 세션에 값이 안넣어진다.
    // /h3 -> /h2 -> /h1 순으로 해야 세션에 값이 생성된다.
    // 이런 상황에서는 /h1 핸들러를 호출하는 시점에 세션이 이미 생성되어 있어야 한다. 이를 달성하기 위해 아래와 같이 할 수 있다.
    //    1. 따라서 파라미터로 HttpSession 을 받아야한다. 그래야 getSession() 이 호출된 상태로 실행되기 때문이다.
    //    2. 또는 앱 시작 시 JSP 페이지를 먼저 띄운다. (JSP 페이지는 servlet 으로 변환 시 HttpSession 을 빌트인으로 만들기 때문)
  }
