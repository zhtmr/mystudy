...
public class Exam0630 {
  public static void main(String[] args) {
    ...
    Worker3 w3 = new Worker3("안중근", job2);    // 다른 인스턴스 사용한다. critical section 없다.
    Worker4 w4 = new Worker4("김구", job1);
  ...
  static class Job {   // 메소드가 서로 달라도 같은 critical section 을 공유한다.
  ...
  static class Worker4 extends Thread {
    ...
    public void run() {
      try {
        job.play1(getName());  // worker4 는 play1 호출
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
